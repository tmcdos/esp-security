...
int ICACHE_FLASH_ATTR httpdFindArg(char *line, char *arg, char *buff, int buffLen) {
	...
	while(p != NULL && *p != '\n' && *p != '\r' && *p != 0) {
		...
		if (os_strncmp(p, arg, os_strlen(arg)) == 0 && p[strlen(arg)] == '=') {
			...
			//NODE_DBG("findArg: val %s len %d\n", p, (e-p));
			return httpdUrlDecode(p, (e-p), buff, buffLen);
		}
		p = (char*)os_strstr(p, "&");
		if (p != NULL) p += 1;
	}
	NODE_DBG("Finding %s in %s: Not found :/\n", arg, line);
	return -1; //not found
}
...
int ICACHE_FLASH_ATTR httpdSend(HttpdConnData *conn, const char *data, int len) {
  ...
  if (conn->priv->sendBuffLen + len>conn->priv->sendBuffMax) {
		NODE_DBG("%s ERROR! httpdSend full (%d of %d)\n",
...
void ICACHE_FLASH_ATTR httpdFlush(HttpdConnData *conn) {
	if (conn->priv->sendBuffLen != 0) {
		...
		if (status != 0) {
			NODE_DBG("%s ERROR! espconn_sent returned %d, trying to send %d to %s\n", 
...
static void ICACHE_FLASH_ATTR httpdSentCb(void *arg) {
  ...
  httpdSetOutputBuffer(conn, sendBuff, sizeof(sendBuff));
	...
	if (conn->cgi == NULL) { //Marked for destruction?
		//NODE_DBG("Closing 0x%p/0x%p->0x%p\n", arg, conn->conn, conn);
	...
	if (r == HTTPD_CGI_NOTFOUND /*|| r == HTTPD_CGI_AUTHENTICATED*/) {
		NODE_DBG("%s ERROR! CGI returns code %d after sending data! Bad CGI!\n", connStr, r);
		conn->cgi = NULL; //mark for destruction.
	}
	httpdFlush(conn);
}
...
static void ICACHE_FLASH_ATTR httpdProcessRequest(HttpdConnData *conn) {
    ...
    if (conn->cgi == NULL) {
      ...
      if (conn->post) conn->post->len = 0; // skip any remaining receives
		  ...
		  if (r == HTTPD_CGI_NOTFOUND /*|| r == HTTPD_CGI_AUTHENTICATED*/) {
        NODE_DBG("%shandler for %s returned invalid result %d\n", connStr, conn->url, r);
      }
...
static void ICACHE_FLASH_ATTR httpdParseHeader(char *h, HttpdConnData *conn) {
	...
	if (first_line) {
		...
		*e = 0; //terminate url part
		...
		//NODE_DBG("Mallocced buffer for %d + 1 bytes of post data.\n", conn->post->buffSize);
...
static void ICACHE_FLASH_ATTR httpdConnectCb(void *arg) {
	...
	//NODE_DBG("Con req, conn=%p, pool slot %d\n", conn, i);
	if (i == MAX_CONN) {
		NODE_DBG("%s HTTP, conn pool overflow!\n", connStr);
