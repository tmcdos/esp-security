...
static char ICACHE_FLASH_ATTR console_prev(void) {
...
ajaxBridgePort(HttpdConnData *connData) {
  ...
  int b;
  char buff[8];
	int len = httpdFindArg(connData->getArgs, "port", buff, sizeof(buff));
...
ajaxConsoleBaud(HttpdConnData *connData) {
	...
	os_sprintf(buff, "{\"rate\": %d,\"port\":%d}", flashConfig.baud_rate, flashConfig.bridge_port);
