...
var pinPresets = {
  // array: r1, r2, r3, r4, swap, rxpup
//  "esp-01":       [  0, -1, 2, -1, 0, 1 ],
//  "esp-12":       [ 12, 14, 0,  2, 0, 1 ],
//  "esp-12 swap":  [  1,  3, 0,  2, 1, 1 ],
//  "esp-bridge":   [ 12, 13, 0, 14, 0, 0 ],
//  "wifi-link-12": [  1,  3, 0,  2, 1, 0 ],
  "esp-bridge":   [ 13, 12, 14, 16, 0, 0 ],
};

pin_num = [16, 5, 4, 0, 2, 14, 12, 13, 15, 3, 1, 9,	10];
num_pin = [3, 10, 4, 9, 2, 1, -1, -1, -1, 11, 12, -1, 6, 7, 5, 8, 0];
pin_name = {/*'disabled':-1,*/ 'GPIO_0':3, 'GPIO_1/TX0':10, 'GPIO_2/TX1':4, 'GPIO_3/RX0':9, 
  'GPIO_4':2, 'GPIO_5':1, 'GPIO_12':6, 'GPIO_13':7, 'GPIO_14':5, 'GPIO_15':8, 'GPIO_16':0};
...
//    console.log("apply preset:", v, pp);
    ...
    $("#pin-r1").value = num_pim[pp[0]];
    $("#pin-r2").value = num_pin[pp[1]];
    $("#pin-r3").value = num_pin[pp[2]];
    $("#pin-r4").value = num_pin[pp[3]];
    $("#pin-swap").value = pp[4];
...
function displayPins(resp) {
  function createSelectForPin(name, v) {
    ...
    for(i in pin_name) 
    {
      ...
      opt.value = pin_name[i];
      opt.text = i;
      if (pin_name[i]==v) opt.selected = true;
      sel.appendChild(opt);
    }
  ...
  createSelectForPin("r1", resp["r1"]);
  createSelectForPin("r2", resp["r2"]);
  createSelectForPin("r3", resp["r3"]);
  createSelectForPin("r4", resp["r4"]);
...
function setPins(ev) {
  ...
  var url = "/pins";
  var sep = "?";
  ["r1", "r2", "r3", "r4", "swap"].forEach(function(p) {
...
function displayRele(resp) {
  Object.keys(resp).forEach(function(v) {
    po = $("#"+v.replace("relay","state"));
    if(po) po.innerHTML = (resp[v] ? "ON" : "OFF");
	  bt = $("#"+v);
	  if(bt) bt.className = 'btn '+(resp[v] ? "btn-yes" : "btn-no");
	});
}

function fetchRele() {
  ajaxJson("GET", "/relay", displayRele, function() {
    window.setTimeout(fetchRele, 1000);
  });
}

function setRele(name,val) {
  ajaxJson("POST", "/relay?"+name+"="+val, function(resp) {
		showNotification(name+" changed to " + (resp[name] ? "ON" : "OFF"));
		displayRele(resp);
	}, function() {
		showWarning("Failed to change state of "+name);
	});
}

function defRele() {
  ajaxReq("POST", "/relay_def", function(resp) {
