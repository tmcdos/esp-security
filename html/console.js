...
function updateText(resp) {
  ...
  if (resp != null && resp.len > 0) 
  {
    ...
    var leng = resp.text.length;
    var s = '';
    for(var i=0; i<leng; i++)
    {
      var code = resp.text.charCodeAt(i);
      if(code==9 || code==10 || code==13 || (code >= 32 && code < 127)) s = s + String.fromCharCode(code);
        else s = s + ' $' + code.toString(16) + ' ';
    }
    el.innerHTML = el.innerHTML.concat(s+"\n");
