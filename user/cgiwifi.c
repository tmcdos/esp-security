...
void ICACHE_FLASH_ATTR wifiAddStateChangeCb(WifiStateChangeCb cb) {
  ...
  NODE_DBG("WIFI: max state change cb count exceeded\n");
}
...
void ICACHE_FLASH_ATTR wifiScanDoneCb(void *arg, STATUS status) {
	...
	if (status!=OK) {
		NODE_DBG("wifiScanDoneCb status=%d\n", status);
	...
	cgiWifiAps.noAps=n;
	NODE_DBG("Scan done: found %d APs\n", n);
	...
	while (bss_link != NULL) {
		if (n>=cgiWifiAps.noAps) {
			...
			NODE_DBG("Huh? I have more than the allocated %d aps!\n", cgiWifiAps.noAps);
			break;
		}
		//Save the ap data.
...
static void ICACHE_FLASH_ATTR resetTimerCb(void *arg) {
	...
	NODE_DBG("Wifi check: mode=%s status=%d\n", wifiMode[m], x);
  if (m == 2) return; // 2=AP, in AP-only mode we don't do any auto-switching
    ...
    // if we got an IP we could switch to STA-only...
		if (m != 1) { // 1 = STA
			...
			NODE_DBG("Wifi got IP. Going into STA mode..\n");
			wifi_set_opmode(1);
    ...
    // we don't have an IP address
		if (m != 3) {
			NODE_DBG("Wifi connect failed. Going into STA+AP mode..\n");
			wifi_set_opmode(3);
      wifi_softap_set_config(&apconf);
		}
		log_uart(true);
		//NODE_DBG("Enabling/continuing uart log\n");
...
static void ICACHE_FLASH_ATTR reassTimerCb(void *arg) {
	NODE_DBG("Wifi changing association\n");
...
int ICACHE_FLASH_ATTR cgiWiFiConnect(HttpdConnData *connData) {
  ...
  if(mode == 2){
      jsonHeader(connData, 400);
      httpdSend(connData, "Can't associate to AP in SoftAP mode", -1);
      return HTTPD_CGI_DONE;
  }
	...
	if (el > 0 && pl >= 0) {
		...
		NODE_DBG("Wifi try to connect to AP %s with pass %s\n", essid, passwd);
...
static void ICACHE_FLASH_ATTR debugIP() {
	...
	if (wifi_get_ip_info(0, &info)) {
		NODE_DBG("\"ip\": \"%d.%d.%d.%d\"\n", IP2STR(&info.ip.addr));
		NODE_DBG("\"netmask\": \"%d.%d.%d.%d\"\n", IP2STR(&info.netmask.addr));
		NODE_DBG("\"gateway\": \"%d.%d.%d.%d\"\n", IP2STR(&info.gw.addr));
...
void ICACHE_FLASH_ATTR configWifiIP() {
	if (flashConfig.staticip == 0) {
		...
		NODE_DBG("Wifi uses DHCP, hostname=%s\n", flashConfig.hostname);
	} else {
		...
		NODE_DBG("Wifi uses static IP %d.%d.%d.%d\n", IP2STR(&ipi.ip.addr));
	}
#ifdef DEBUGIP
	debugIP();
#endif
...
int ICACHE_FLASH_ATTR cgiApSettingsChange(HttpdConnData *connData) {
    ...
    if(len != 0)
    {
      ...
      if(len>0)
      {
        ...
        else
        {
          ...
          NODE_DBG("Forcing AP authmode to WPA_WPA2_PSK\n");
          apconf.authmode = 4;
        }
      }
      else
    ...
    else apconf.authmode = 0;
    NODE_DBG("Setting AP authmode=%d\n", apconf.authmode);
...
int ICACHE_FLASH_ATTR cgiWiFiSetMode(HttpdConnData *connData) {
  ...
  int next_mode = atoi(buff);
    ...
    NODE_DBG("Wifi switching to mode %d\n", next_mode);
...
//This cgi changes the SSID / password for AP and stores them in Flash
int ICACHE_FLASH_ATTR cgiWiFiSetPass(HttpdConnData *connData) {
	int len_ssid,len_pass;
	char sid[32],pass[64];

	if (connData->conn==NULL) return HTTPD_CGI_DONE; // Connection aborted. Clean up.

	len_ssid=httpdFindArg(connData->getArgs, "ssid", sid, sizeof(sid));
	len_pass=httpdFindArg(connData->getArgs, "pass", pass, sizeof(pass));
	...
	if (len_ssid!=0) 
	{
		NODE_DBG("Changing AP SSID to '%s'\n", sid);
		memcpy(&flashConfig.ssid, &sid, sizeof(flashConfig.ssid));
		configSave();
		jsonHeader(connData, 200);
	}
	if (len_pass!=0) 
	{
		NODE_DBG("Changing AP password to '%s'\n", pass);
		memcpy(&flashConfig.password, &pass, sizeof(flashConfig.password));
		configSave();
		jsonHeader(connData, 200);
	}
	return HTTPD_CGI_DONE;
...
int ICACHE_FLASH_ATTR checkString(char *str){
  for(int i=0; i<os_strlen(str); i++) {
    ...
    if (str[i] < '!' || str[i] > '~') {
      NODE_DBG("Error: String has non alphanumeric chars\n");
      return 0;
    }
  }
  return 1;
...
void ICACHE_FLASH_ATTR wifiInit() {
  ...
  if (os_strlen((char*)stconf.ssid) == 0 && os_strlen((char*)stconf.password) == 0) {
    ...
    wifi_set_opmode(3);
    NODE_DBG("Wifi pre-config trying to connect to AP %s with pass %s\n",(char*)stconf.ssid, (char*)stconf.password);
  ...
  // Debug info
  NODE_DBG("Wifi Soft-AP parameters change: %s\n",softap_set_conf? "success":"fail");
