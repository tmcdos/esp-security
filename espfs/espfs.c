...
int ICACHE_FLASH_ATTR espFsFlags(EspFsFile *fh) {
	if (fh == NULL) {
		NODE_DBG("File handle not ready\n");
		return -1;
	}
...
EspFsFile ICACHE_FLASH_ATTR *espFsOpen(char *fileName) {
	...
	while(1) {
		...
		if (h.magic!=ESPFS_MAGIC) {
			NODE_DBG("Magic mismatch. EspFS image broken.\n");
			return NULL;
		}
		if (h.flags&FLAG_LASTFILE) {
			//NODE_DBG("End of image.\n");
			return NULL;
		}
		//Grab the name of the file.
		p+=sizeof(EspFsHeader);
		os_memcpy(namebuf, p, sizeof(namebuf));
//		NODE_DBG("Found file '%s'. Namelen=%x fileLenComp=%x, compr=%d flags=%d\n",
//				namebuf, (unsigned int)h.nameLen, (unsigned int)h.fileLenComp, h.compression, h.flags);
		if (os_strcmp(namebuf, fileName)==0) {
			...
			if (h.compression==COMPRESS_NONE) {
				...
				r->posComp++;
				//NODE_DBG("Heatshrink compressed file; decode parms = %x\n", parm);
				dec=heatshrink_decoder_alloc(16, (parm>>4)&0xf, parm&0xf);
				r->decompData=dec;
#endif
			} else {
				NODE_DBG("Invalid compression: %d\n", h.compression);
				return NULL;
			}
			return r;
		}
...
//		NODE_DBG("Reading %d bytes from %x\n", len, (unsigned int)fh->posComp);
		memcpyAligned(buff, fh->posComp, len);
		fh->posDecomp+=len;
		fh->posComp+=len;
//		NODE_DBG("Done reading %d bytes, pos=%x\n", len, fh->posComp);
		return len;
#ifdef ESPFS_HEATSHRINK
...
//			NODE_DBG("Elen %d rlen %d d %d pd %ld fdl %d\n",elen,rlen,decoded, fh->posDecomp, fdlen);

			if (elen == 0) {
				if (fh->posDecomp == fdlen) {
//					NODE_DBG("Decoder finish\n");
